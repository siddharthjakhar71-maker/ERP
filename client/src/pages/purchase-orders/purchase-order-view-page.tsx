...
export const PurchaseOrderViewPage = () => {
  ...
  return (
    <PurchaseOrderPageLayout
      ...
      actions={purchaseOrder.data ? <div className="flex flex-wrap gap-3"><PurchaseOrderDownloadPdfButton purchaseOrder={purchaseOrder.data} /><Link to="/grn/new" search={{ purchaseOrderId }} className={buttonVariants({ variant: 'outline' })}>Create GRN</Link><Link to="/purchase-orders/$purchaseOrderId/edit" params={{ purchaseOrderId }} className={buttonVariants()}><Pencil className="mr-2 h-4 w-4" />Edit purchase order</Link></div> : null}
    >
